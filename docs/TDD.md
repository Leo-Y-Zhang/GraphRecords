# TDD — GraphRecords: the bishop-to-rook engine as built

**Status:** built (Phase 1 complete)
**Date:** 2026-08-03 · **PRD:** [PRD.md](PRD.md) · **Repo:** `GreenPandaTech/GraphRecords`

Derived from the code, not from the README. Sections of the standard template that
have no counterpart here (RLS policies, security-definer functions, `anon` grants)
are cut rather than padded: this is an offline computation with no server, no
database and no authenticated surface. What it does have is a **publication**
trust boundary, which is the section that replaces them.

## Approach

A bishop never changes the parity of `row + col`, so the n X n bishop graph is the
disjoint union of two components, one per colour. On a single colour the map

    phi(r, c) = ((r + c) // 2, (r - c + n) // 2)

is injective and sends "shares an anti-diagonal" to "agrees in x" and "shares a
main diagonal" to "agrees in y". So the single-colour bishop graph **is** the rook
graph on the image cells. That is the whole engine: everything else is a
consequence.

Two consequences do the work.

- **Connectivity collapses.** All cells sharing a class are mutually adjacent, so
  a cell subset is connected in the rook graph iff the bipartite graph it induces
  between x-classes and y-classes is connected. Counting moves from `O(2^|cells|)`
  to a sweep over `O(n)` classes per side.
- **Domination collapses.** A cell's closed neighbourhood is the *whole* of its
  x-class together with the *whole* of its y-class, so a set dominates iff its
  `(x-support, y-support)` is a vertex cover of the cell bipartite graph.
  Domination depends only on which classes are touched, never on how many cells
  are used inside them.

The sharp rule that came out of this, and the most transferable result in the
repo: **predicates that depend only on which classes are occupied collapse;
predicates that need class populations do not.** Total domination uses the open
neighbourhood, so a chosen cell does not cover itself and the sweep must
distinguish "one cell here" from "two or more". That single extra level costs
seven terms of reach (n=18 down to n=11).
...
There is no database. The state that matters is (a) two in-memory encodings that
were chosen by measurement, and (b) four persisted JSON/text artefacts.
...
Each x-class meets a **contiguous** y-interval. That is the single fact all the
...
directions — the transposed sweep would be unsound otherwise.
...
**Nullability analogue.** Two "legacy row" cases exist and both reach real runs:
`nx == 0 or ny == 0` (the white board at n=1 has no cells) and the empty set.
The counters disagree deliberately: `frontier_connected` returns **0** there
(no non-empty connected subgraph exists) while `dominating_sets` returns **1**
(the empty set dominates the empty graph). Getting either backwards shifts a
whole sequence by one index, which is why the gate checks against published data
by true `n` and not by position.
...
are **not** uniform across this family and never assume otherwise.
...
Two contracts worth stating because they are load-bearing:
...
- `peeling_machinery` is exposed rather than kept private so that
  `cds_peeling.py` can reuse the *already cross-checked* peeling path instead of
  transcribing a second copy that could drift from it. Its `C(X, Y)` is summed
  over the dominating supports, which are exactly those with
...
1. **oeis.org, read-only.** `tools/probe_upstream_bfiles.py` and
   `tools/build_submission_pack.py` fetch published b-files over HTTPS with a
   45 s timeout and 0.4 s between requests. Both fail **closed**: an HTML body,
   a rate limit, a dropped connection or an empty parse is reported as
   **UNKNOWN, never as absent**. This is the most important design decision in
   the repo's plumbing, because "we could not tell whether anything is published"
   silently becoming "nothing is published" is precisely what lets an EXTENSIONS
   line take credit for another author's terms. `build_submission_pack.py` raises
   `SystemExit` rather than writing a pack it cannot justify.
2. **b-file bytes.** The OEIS spec requires LF-only ASCII with no BOM and a
   trailing newline. A Windows checkout will rewrite these to CRLF and silently
   violate it, so `.gitattributes` pins `OEIS-upload/*.txt` to `eol=lf` and
   `tests/test_staged_bfiles.py` asserts the bytes. A fresh clone was verified to
   still produce LF-only files.
3. **Publication.** The only irreversible action in the system is a human pasting
   a b-file into OEIS. Nothing in this repository can perform it; there are no
   credentials anywhere in it, by design.

## Migrations

None — no schema. The nearest equivalent is refreshing `data/upstream_bfiles.json`,
which is additive, idempotent and reversible by `git revert`. It has one rule
with the same force as "never edit an applied migration": **run the probe before
staging, never after**, and never treat a stale snapshot as current.
...
| Upstream probe cannot reach OEIS | The operator, immediately | Probe prints UNKNOWN and `main()` returns 1; the pack builder raises `SystemExit` | Re-run when the network is available, or `--offline` for a pack with no EXTENSIONS lines. |
| b-files rewritten to CRLF by a Windows checkout | Nobody, until OEIS rejects the file | `.gitattributes` + byte-level test | Re-checkout; the test fails first. |
| An overnight run exhausts RAM and freezes the machine | The operator, painfully | `memguard.start()` polls free physical RAM from a daemon thread and calls `os._exit(3)` on breach | Nothing to undo — losing the run is the intended outcome. Everything printed before the breach is valid. |
| A term takes longer than the session | The operator | `--budget` stops a sequence once a single term exceeds it | Resume at a lower limit. |
...
killing a healthy job on a transient read error is the worse failure.

## Rollback

- **Code and data:** `git revert`. There is no deployed surface, no consumer, no
  state outside the repository. Rollback is seconds.
- **Staged b-files:** delete the file. They are inert until pasted.
- **The one irreversible step is submission**, and it is outside this system by
  construction. An accepted OEIS entry cannot be retracted the way a deploy can
  be rolled back; a wrong term propagates into a public reference other people
...
checks + 372 tests, exit 0 (locally, 2026-08-03; 372 tests in 135 s).
...
| **L3** | Frontier DP vs the peeling counter, n=1..8, both colours — for connected subgraphs and, since 2026-08-01, connected dominating sets. Shares no code path beyond the class grid. |
...
- `connected_domination.py` fixes `REQ_WIDTH = 4`, so the requirement mask holds
  32 y-classes. The bishop board reaches ny ~ n, and this counter's practical
  ceiling is n=10-11, so the limit is far out of reach — but it is a hard limit,
  not a soft one, and would raise `OverflowError` rather than return a wrong
  answer.
- Everything here is **verified computation, not proof**. The strongest claim the
  ...
  checking agree. Section 6 of `PAPER.md` says this and must not be softened.
...
1. `boards.py` + `brute.py` — definitions and a reference nothing else may
   contradict.
...
4. `connected.py` frontier DP with stranded-block pruning — the production engine.
...
8. `tools/probe_upstream_bfiles.py` + the staging tests — added *after* the
