...
Why this exists. The external watchdog (`tools/ram_guard.ps1`) has to be handed a
PID, and capturing that PID races the process it is meant to protect: twice
tonight the guard silently failed to arm and a job climbed to 4.4 GB unprotected
while free RAM fell to 1.5 GB. A guard that is only *usually* armed is not a
guard.
...
alternative is paging the whole machine, which on this box has meant a freeze.
